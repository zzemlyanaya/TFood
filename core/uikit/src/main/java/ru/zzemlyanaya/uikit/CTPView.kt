...
import ru.zzemlyanaya.tfood.uikit.R
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mCurrentRadianWake > mCurrentRadianBed) {
            canvas.drawArc(
                ...
                Math.toDegrees((2 * Math.PI.toFloat()).toDouble()).toFloat() - Math.toDegrees(
                    mCurrentRadianWake.toDouble()
                ).toFloat() + Math.toDegrees(mCurrentRadianBed.toDouble()).toFloat(),
            ...
            canvas.drawArc(
                rect,
                Math.toDegrees(mCurrentRadianWake.toDouble()).toFloat(),
                Math.toDegrees(mCurrentRadianBed.toDouble()).toFloat() - Math.toDegrees(
                    mCurrentRadianWake.toDouble()
                ).toFloat(),
                false, mProgressPaint
            )
        ...
        if (ismInCircleButtonBed) {
            ...
            canvas.drawBitmap(
                dawnBitmap,
                mCx - mCircleButtonRadius / 2,
                butY - mCircleButtonRadius / 2,
                mIconsPaint
            )
            canvas.restore()
            canvas.save()
